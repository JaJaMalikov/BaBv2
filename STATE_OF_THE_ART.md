...
- Refactor State Applier: extraction de l'application des états keyframes vers les items graphiques dans `ui/state_applier.py` via `StateApplier`. `AppController` appelle désormais directement `scene_controller.apply_puppet_states` et `apply_object_states`.
