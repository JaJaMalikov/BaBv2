...
export class SceneModel {
  ...
  addKeyframe(index: number): Keyframe {
    ...
    for (const [puppetName, puppet] of Object.entries(this.puppets)) {
      ...
      for (const [memberName, member] of Object.entries(puppet.members)) {
        state[memberName] = { pivot: member.pivot, zOrder: member.zOrder };
      }
      kf.puppets[puppetName] = state;
    }
    this.keyframes = Object.fromEntries(
      Object.entries(this.keyframes).sort(([a], [b]) => Number(a) - Number(b))
    );
    return kf;
  }
