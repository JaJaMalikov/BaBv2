...
export class PuppetMember {
  ...
  constructor(name: string, parent: PuppetMember | null = null, pivot: [number, number] = [0,0], bbox: [number, number, number, number] = [0,0,0,0], zOrder = 0) {
    this.name = name;
    this.parent = parent;
    this.children = [];
    this.pivot = pivot;
...
export class Puppet {
  ...
  buildFromSvg(svgLoader: SvgLoader | null, parentMap: Record<string, string | null>, pivotMap?: Record<string, string>, zOrderMap?: Record<string, number>) {
    this.childMap = computeChildMap(parentMap);
    const groups = svgLoader ? svgLoader.getGroups() : Object.keys(parentMap);
    for (const groupId of groups) {
      ...
      const pivot = svgLoader?.getPivot(pivotGroup) ?? [0,0];
      const z = zOrderMap?.[groupId] ?? 0;
      this.members[groupId] = new PuppetMember(groupId, null, pivot as [number, number], bbox as [number, number, number, number], z);
    }
    for (const [child, parent] of Object.entries(parentMap)) {
  ...
  printHierarchy(member: PuppetMember | null = null, indent = '') {
    if (!member) {
