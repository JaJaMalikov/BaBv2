...
class SettingsDialog(QDialog):
    ...
    def __init__(self, parent: Optional[Any] = None) -> None:
        ...
        self.lib_w = QSpinBox(); self.lib_h = QSpinBox()
        self.insp_w = QSpinBox(); self.insp_h = QSpinBox()
        for sp in (self.lib_w, self.lib_h, self.insp_w, self.insp_h):
            sp.setRange(150, 2000)
        lib_size_layout = QHBoxLayout(); lib_size_layout.addWidget(self.lib_w); lib_size_layout.addWidget(self.lib_h)
        insp_size_layout = QHBoxLayout(); insp_size_layout.addWidget(self.insp_w); insp_size_layout.addWidget(self.insp_h)
        ...
        self.lib_x = QSpinBox(); self.lib_y = QSpinBox()
        self.insp_x = QSpinBox(); self.insp_y = QSpinBox()
        self.cust_x = QSpinBox(); self.cust_y = QSpinBox()
        self.cust_w = QSpinBox(); self.cust_h = QSpinBox()
        ...
        lib_pos_layout = QHBoxLayout(); lib_pos_layout.addWidget(self.lib_x); lib_pos_layout.addWidget(self.lib_y)
        insp_pos_layout = QHBoxLayout(); insp_pos_layout.addWidget(self.insp_x); insp_pos_layout.addWidget(self.insp_y)
        cust_pos_layout = QHBoxLayout(); cust_pos_layout.addWidget(self.cust_x); cust_pos_layout.addWidget(self.cust_y)
        ...
        cust_size_layout = QHBoxLayout(); cust_size_layout.addWidget(self.cust_w); cust_size_layout.addWidget(self.cust_h)
        ...
        # Icon size
        self.icon_size_spin = QSpinBox(); self.icon_size_spin.setRange(16, 128); self.icon_size_spin.setSingleStep(4)
        ...
        self.list_main_order = QListWidget(); self.list_main_order.setDragDropMode(QAbstractItemView.InternalMove)
        self.list_quick_order = QListWidget(); self.list_quick_order.setDragDropMode(QAbstractItemView.InternalMove)
        self.list_custom_order = QListWidget(); self.list_custom_order.setDragDropMode(QAbstractItemView.InternalMove)
        ...
        over_scroll = QScrollArea(); over_scroll.setWidgetResizable(True); over_scroll.setWidget(tab_over_inner)
        ...
        self.lbl_key = QLabel("—")
        self.lbl_path = QLineEdit(); self.lbl_path.setReadOnly(True)
        ...
        row_btns = QHBoxLayout(); row_btns.addWidget(self.btn_pick_icon); row_btns.addWidget(self.btn_reset_icon); row_btns.addStretch(1)
        row_wrap = QWidget(); row_wrap.setLayout(row_btns)
        ...
        icons_scroll = QScrollArea(); icons_scroll.setWidgetResizable(True); icons_scroll.setWidget(tab_icons_inner)
        tabs.addTab(icons_scroll, "Icônes")

        # --- Tab: Onion ---
        tab_onion = QWidget()
        onion_form = QFormLayout(tab_onion)
        self.prev_count = QSpinBox(); self.prev_count.setRange(0, 10)
        self.next_count = QSpinBox(); self.next_count.setRange(0, 10)
        self.opacity_prev = QDoubleSpinBox(); self.opacity_prev.setRange(0.0, 1.0); self.opacity_prev.setSingleStep(0.05)
        self.opacity_next = QDoubleSpinBox(); self.opacity_next.setRange(0.0, 1.0); self.opacity_next.setSingleStep(0.05)
        ...
        controls = QWidget(); controls_layout = QFormLayout(controls)
        controls_layout.setLabelAlignment(Qt.AlignRight)
        # Presets
        self.preset_combo = QComboBox(); self.preset_combo.addItems(["Light", "Dark", "High Contrast", "Custom"])
        controls_layout.addRow("Preset:", self.preset_combo)
        # Color pickers (edit + button)
        self._swatches = {}
        def mk_color_row(title: str):
            le = QLineEdit(); btn = QPushButton("…"); btn.setFixedWidth(28)
            # Swatch showing current color
            sw = QLabel(); sw.setFixedSize(24, 16)
        ...
        # Numeric controls
        self.opacity_spin = QSpinBox(); self.opacity_spin.setRange(0, 100); self.opacity_spin.setSuffix(" %")
        self.radius_spin = QSpinBox(); self.radius_spin.setRange(0, 24); self.radius_spin.setSuffix(" px")
        self.font_spin = QSpinBox(); self.font_spin.setRange(8, 18); self.font_spin.setSuffix(" pt")
        ...
        actions_wrap = QWidget(); actions_wrap.setLayout(actions)
        ...
        tb_bar = QHBoxLayout()
        tb1 = QToolButton(); tb1.setText("Outil 1"); tb1.setCheckable(True); tb1.setChecked(True)
        tb2 = QToolButton(); tb2.setText("Outil 2")
        tb3 = QToolButton(); tb3.setText("Outil 3")
        tb_bar.addWidget(tb1); tb_bar.addWidget(tb2); tb_bar.addWidget(tb3); tb_bar.addStretch(1)
        tb_wrap = QWidget(); tb_wrap.setLayout(tb_bar)
        gb_form.addRow("Outils:", tb_wrap)

        # Inputs to show line edit + combo focus/accent
        self.prev_input = QLineEdit(); self.prev_input.setPlaceholderText("Texte…")
        combo = QComboBox(); combo.addItems(["Premier", "Second", "Troisième"])
        ...
        style_scroll = QScrollArea(); style_scroll.setWidgetResizable(True); style_scroll.setWidget(tab_style_inner)
